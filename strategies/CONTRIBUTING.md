...
## Rules
- **Schema validation is not adjudication.** A valid pack has no portfolio dollars
  or court verdict/date. It may supply a narrative and deck alongside a held thesis,
  but cannot replace its custody values, positions or adjudicated verdict. The
  author-supplied edge is a claim for diligence, not an independently verified label.
- **Keep the deck inside the pack.** IDs are lowercase slugs of at most 80
  characters, beginning with a letter or digit. The deck must be a real file inside
  the pack after resolving paths and symlinks. Traversal to neighboring files fails.
- **Describe risk as a hypothesis.** A bucket is an intended role, not a guarantee
  of safety or a reservation of money for a goal. The app estimates stress only
  when the thesis symbols map to actual modeled holdings; otherwise it says unknown.
