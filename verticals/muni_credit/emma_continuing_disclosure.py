"""emma_continuing_disclosure — per-CUSIP EMMA continuing-disclosure (15c2-12) history.

PROMOTED from the 2026-06-11 pre-trade DD scratch scanner (_cd_scan_p1.py) after it verified the
endpoint on 25 live names: POST /Security/ContinuingDisclosurePartialView with
{SecurityId: <plaintext CUSIP>, SelectedPredefinedDateRange: 'All'} once the Disclaimer6 cookie is
set (fetch_cd handles the dance). parse_cd_html returns filing rows (group, desc, posted/period
dates) + section names.

WHY IT'S A BRAIN-LEVEL SOURCE. 15c2-12 event notices are the FASTEST honesty channel in munis
(days, vs annual for everything else): failure-to-provide-annual-information, rating changes,
reserve draws, payment delinquencies, defeasance/redemption notices. Two triage rules learned
on the live basket:
  1. ATWATER PATTERN (masking): on-time OPERATING-data filings can mask chronically late AUDITED
     financials — check audit currency separately from filing currency (Atwater: 3 consecutive
     failure-to-file notices 2024-26 while the operating report was punctual every year).
  2. WRAP-EVENT NOISE: bond-insurer notices (AGM merger/rating items) fire as event notices on
     every wrapped CUSIP — they are insurer-channel, not issuer credit; classify separately.
Consumers: cd_monitor.py (holdings delta watch), pre-trade DD scans, late_filing detector.
"""
