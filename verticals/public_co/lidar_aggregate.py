"""
Aggregate the 5 blinded subagent outputs for the lidar / ADAS cohort into
a single cohort matrix + forward-prediction report.
"""
...
from .lidar_cohort import COHORT, CUTOFF

HERE  = Path(__file__).parent
LOCAL = HERE / "data" / "_local"
OUT   = HERE / "data" / "_lidar_cohort"
...
def write_report(matrix: list[dict]) -> None:
    ...
    w("# Lidar / ADAS Cohort — Blinded Forward-Test Predictions (v1)\n")
    w(f"_Generated {datetime.utcnow().isoformat()}Z — cutoff {CUTOFF}_\n")

    w("## Methodology\n")
    w(
        "Five US-listed automotive-lidar / ADAS sensor pure-plays: LAZR, "
        "AEVA, OUST, INVZ (Israeli foreign private issuer, 20-F filer), "
        "MVIS. Each ticker was analyzed by an isolated, **blinded** Claude "
        "Code subagent with no outcome labels, no WebSearch / WebFetch "
        "access, no shared context with other tickers. Each subagent:\n\n"
        "1. Read the company's most recent pre-cutoff filing (most recent "
        "10-K for US filers, 20-F for INVZ, S-1 if no 10-K yet, 10-Q for "
        "newer interim disclosures).\n"
        "2. Extracted 5-8 specific, testable, falsifiable factual claims.\n"
        "3. Picked M-source queries from the catalog (EDGAR full-text "
        "against counterparty CIKs for OEM-design-win cross-check, USPTO "
        "ODP for FMCW / MEMS / optics patent claims, EPA FRS for "
        "manufacturing claims, USAspending for federal-contract claims).\n"
        "4. Executed queries and scored each claim PASS / "
        "MODERATE_UNDERDELIVERY / SEVERE_UNDERDELIVERY / "
        "RED_FLAG_NEGATIVE / UNVERIFIABLE.\n\n"
        "**Lidar-specific calibration emphasis:** the **design-win stage "
        "ladder** discipline (Heuristic 3 with extra weight) — RFQ → "
        "selection → design-win awarded → A/B/C-sample → tooling-complete "
        "→ Start-of-Production (SOP) → series production at volume. "
        "Conflating these stages in marketing-tier language while the "
        "financial-statement notes reveal a lower stage is the central "
        "evasion pattern. Counterparty-disclosure threshold (Rule 7) "
        "applied hard: claimed series-production contracts with Tier-1 "
        "OEMs should appear in the OEM's 10-K / 20-F; absence is a hard "
        "contradiction.\n"
    )
    ...
    w(
        ...
        "It does not mean the company is lying. Multiple legitimate reasons "
        "exist for OEM-counterparty-disclosure absence (sub-tier supply "
        "through a Tier-1 integrator that obscures the lidar OEM name; "
        "platforms in pre-production not yet material to the OEM; "
        "confidential supplier-agreement clauses).\n"
        "- **Not a trade recommendation.** Lidar names are mostly micro/"
        "small-cap and shortability is variable. See prior "
        "`_distress_sift/PREDICTIONS.md` for shortability methodology.\n"
        "- **Not pre-registered.** For the predictions to count as honest "
        "forward bets they need a cryptographic timestamp. Recommend "
        "committing this report's hash to git or an external timestamp "
        "service.\n"
    )

    w("## Debt and tradeoffs\n")
    w(
        "- **Subagent variance.** Five independent runs produced different "
        "claim selection per ticker. Re-runs with different sampling can "
        "shift the result.\n"
        "- **OEM 20-F counterparty disclosure.** Volvo, Mercedes, BMW, VW, "
        "Stellantis are foreign filers (20-F) — the EDGAR full-text search "
        "covers their US filings (20-F + 6-K) but their primary annual "
        "reports outside the US are not in EDGAR. Counterparty-absence in "
        "20-F is suggestive but not definitive without checking the foreign "
        "filing.\n"
        "- **NHTSA component scope.** NHTSA does not register lidar "
        "suppliers (it registers complete vehicles). Heuristic 5 explicitly "
        "excludes NHTSA queries against the lidar company name.\n"
        "- **Coverage gaps surfaced by subagents (queued for "
        "`connectors_to_add.md`):**\n"
        "  - IATF 16949 / AEC-Q100 automotive-supplier certification "
        "registries (no current connector)\n"
        "  - ISO-26262 functional-safety certification registries\n"
        "  - DOT NHTSA OEM-supplier disclosure database (sub-tier "
        "supplier names in vehicle compliance filings)\n"
    )
