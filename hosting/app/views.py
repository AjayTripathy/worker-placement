...
def research_index(catalog,area=None,entries=(),query='',offset=0,symbol=False):
    if area:
        ...
        search = '<form method="get"><label for="query">Search all research files by ticker or topic</label><input id="query" name="q" value="'+esc(query,quote=True)+'" maxlength="200"><button class="button outline">Search</button></form>'
        rows = ''.join('<li><a href="/app/research/document?' + urlencode({'area': e['area'], 'path': e['path']}) + '">' + esc(e['path']) + '</a><small>' + str(e['size']) + ' bytes</small></li>' for e in entries[offset:offset+100])
        params = {'q': query, 'offset': offset+100}
        if symbol:
            params['match'] = 'symbol'
        next_link = '<a href="/app/research?' + urlencode(params) + '">Next 100 →</a>' if offset+100 < len(entries) else ''
        body = '<h1>Research for ' + esc(query) + '</h1>' + search + '<p>' + str(len(entries)) + ' matching files across the published library. Matches are based on ' + ('ticker tokens in file paths' if symbol else 'file paths') + '; read each source for its date, conclusion and limitations.</p><ul class="document-list">' + rows + '</ul>' + next_link
    else:
        body='<h1>Your research<br>starting point.</h1><p>SignalOS findings, datasets and source code, included with every account. These are research records with their original dates and limitations; inclusion is not an investment recommendation.</p><form method="get"><label for="query">Search all research files by ticker or topic</label><input id="query" name="q" maxlength="200"><button class="button outline">Search</button></form><div class="account-grid">'+''.join('<article><h2>'+esc(a['title'].replace('verticals/','').replace('_',' '))+'</h2><p>'+str(a['files'])+' files · '+str(round(a['bytes']/1048576,1))+' MiB</p><a class="text-link" href="/app/research?area='+esc(a['id'],quote=True)+'">Explore ↗</a></article>' for a in catalog['areas'])+'</div>'
