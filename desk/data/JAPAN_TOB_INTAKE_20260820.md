# Japan TOB / take-private lane — intake 2026-08-20

USD/JPY 159.12 at run. Announced book (spreads) and anticipatory book (fingerprints) are separate and are never summed.
...
|---|---|---|---|---|---|---|---|---|---|
| 2997 | Ｇ－ストレージ王 | エリアリンク | 1,340 | 1,326 | +1.09 | +50 | 8 | QUOTABLE_SMALL | DATED |
| 2162 | ｎｍｓ　ＨＤ | ワールドＨＤ | 540 | 537 | +0.56 | +17 | 12 | THIN | DATED_STEP2 |
| 4722 | フューチャー | 合同会社キーウェスト・ネットワー | 2,451 | 2,446 | +0.20 | +3 | 28 | QUOTABLE | DATED |
| 3276 | ＪＰＭＣ | 株式会社Amsterdam1及び | 2,250 | 2,244 | +0.24 | +2 | 36 | QUOTABLE | DATED |
| 7523 | アールビバン | 株式会社Orsay | 1,900 | 1,899 | +0.05 | +2 | 12 | THIN | DATED |
| 9110 | ユナイテド海 | 郵船 | 10,600 | 10,445 | +1.48 | — | — | QUOTABLE | TIMING_UNBOUNDED |
| 2371 | カカクコム | Kamgras1株式会社 | 3,450 | 3,708 | -6.95 | — | — | QUOTABLE | TIMING_UNBOUNDED |

### 2997 Ｇ－ストレージ王 — OPEN

- **Bidder / purpose**: エリアリンク · GO_PRIVATE · target opinion SUPPORT_AND_RECOMMEND
- **Terms**: offer ¥1,340 vs spot ¥1,326 = +1.09% gross
- **Timing**: DATED — cash expected 2026-08-28 (8d)
...
- **JPY add**: a ¥1,000,000 clip here is $6,285 of ADDITIONAL unhedged JPY on top of the existing Japan book.
- **Taxable**: tender proceeds are a disposition, not a rollover — short-term gain unless held >1y. On a +1.09% gross spread the after-tax return at a ~50% marginal rate is roughly half the headline.
...
- **Timing**: DATED_STEP2 — step-2 cash-out effective 2026-09-01 (12d); delisting 2026-08-28
...
- **Timing**: DATED — cash expected 2026-09-17 (28d)
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥2500M/day
- **JPY add**: a ¥1,000,000 clip here is $6,285 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Timing**: DATED — cash expected 2026-09-25 (36d)
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥421M/day
- **JPY add**: a ¥1,000,000 clip here is $6,285 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Terms**: offer ¥1,900 vs spot ¥1,899 = +0.05% gross
- **Timing**: DATED — cash expected 2026-09-01 (12d)
...
- **JPY add**: a ¥1,000,000 clip here is $6,285 of ADDITIONAL unhedged JPY on top of the existing Japan book.
- **Taxable**: tender proceeds are a disposition, not a rollover — short-term gain unless held >1y. On a +0.05% gross spread the after-tax return at a ~50% marginal rate is roughly half the headline.
...
- **Timing**: TIMING_UNBOUNDED — offer is PRE-CONDITIONAL (competition clearance etc.); the commencement date is NOT fixed, so a computed annualized number would be manufactured. Gross only. SCENARIO ONLY: at 132d to cash (from the bidder's own guided commencement window (2026-11-25)) the gross +1.48% annualizes to +4%/yr — an assumption, NOT a term of the offer, and it goes to zero if the conditions fail.
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥1128M/day
- **JPY add**: a ¥1,000,000 clip here is $6,285 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥1130M/day
- **JPY add**: a ¥1,000,000 clip here is $6,285 of ADDITIONAL unhedged JPY on top of the existing Japan book.
- **Taxable**: tender proceeds are a disposition, not a rollover — short-term gain unless held >1y. On a -6.95% gross spread the after-tax return at a ~50% marginal rate is roughly half the headline.
...
- **8283 ＰＡＬＴＡＣ** — headline +0.30% vs offer ¥6,650. not quotable: last print 13d old — spread struck on a stale tape
- **4171 グローバルＩ** — headline +0.90% vs offer ¥1,680. not quotable: last print 8d old — spread struck on a stale tape
...
- Inputs: annual-report balance sheet, price cached 14d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 135,452,804 issued shares, unit 千株
...
- Minority float ~49% of shares; the controller needs to reach 90% to squeeze out without an EGM.
- Inputs: annual-report balance sheet, price cached 14d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 273,768,922 issued shares, unit 千株
