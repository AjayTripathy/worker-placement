# Japan TOB / take-private lane — intake 2026-08-12

USD/JPY 159.13 at run. Announced book (spreads) and anticipatory book (fingerprints) are separate and are never summed.
...
41 deals tracked in the window; 12 live; 8 with a computable spread.

| target | name | bidder | offer ¥ | spot ¥ | gross % | ann. % | days | liquidity | timing |
|---|---|---|---|---|---|---|---|---|---|
| 4171 | グローバルＩ | 株式会社ユーザベース | 1,680 | 1,665 | +0.90 | +66 | 5 | THIN | DATED_STEP2 |
| 2162 | ｎｍｓ　ＨＤ | ワールドＨＤ | 540 | 536 | +0.75 | +14 | 20 | THIN | DATED_STEP2 |
| 2997 | Ｇ－ストレージ王 | エリアリンク | 1,340 | 1,336 | +0.30 | +7 | 16 | QUOTABLE_SMALL | DATED |
| 4722 | フューチャー | 合同会社キーウェスト・ネットワー | 2,451 | 2,445 | +0.24 | +2 | 36 | QUOTABLE | DATED |
| 3276 | ＪＰＭＣ | 株式会社Amsterdam1及び | 2,250 | 2,244 | +0.27 | +2 | 44 | QUOTABLE | DATED |
| 7523 | アールビバン | 株式会社Orsay | 1,900 | 1,898 | +0.10 | +2 | 20 | THIN | DATED |
| 9110 | ユナイテド海 | 郵船 | 10,600 | 10,430 | +1.63 | — | — | QUOTABLE | TIMING_UNBOUNDED |
...
- **Timing**: DATED_STEP2 — step-2 cash-out effective 2026-08-17 (5d); delisting 2026-08-13
...
- **JPY add**: a ¥1,000,000 clip here is $6,284 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Timing**: DATED_STEP2 — step-2 cash-out effective 2026-09-01 (20d); delisting 2026-08-28
- **Minimum-tender condition**: no floor parsed — treat as UNVERIFIED, not as 'no condition'
- **Squeeze-out**: step 2 = 株式売渡請求 (Companies Act 179): available only at >=90% voting; ~1-2 months post-settlement, no EGM.
...
- **Timing**: DATED — cash expected 2026-08-28 (16d)
...
- **Liquidity (thin-print gate)**: QUOTABLE_SMALL — 20d median turnover ¥62M/day — small clips only
- **JPY add**: a ¥1,000,000 clip here is $6,284 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Terms**: offer ¥2,451 vs spot ¥2,445 = +0.24% gross
- **Timing**: DATED — cash expected 2026-09-17 (36d)
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥3246M/day
- **JPY add**: a ¥1,000,000 clip here is $6,284 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Timing**: DATED — cash expected 2026-09-25 (44d)
...
- **Timing**: DATED — cash expected 2026-09-01 (20d)
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: THIN — 20d median turnover ¥15.8M/day — a quoted spread here is not fillable at size; this is a delisting stub, not a position
- **JPY add**: a ¥1,000,000 clip here is $6,284 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Terms**: offer ¥10,600 vs spot ¥10,430 = +1.63% gross
- **Timing**: TIMING_UNBOUNDED — offer is PRE-CONDITIONAL (competition clearance etc.); the commencement date is NOT fixed, so a computed annualized number would be manufactured. Gross only. SCENARIO ONLY: at 140d to cash (from the bidder's own guided commencement window (2026-11-25)) the gross +1.63% annualizes to +4%/yr — an assumption, NOT a term of the offer, and it goes to zero if the conditions fail.
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥699M/day
- **JPY add**: a ¥1,000,000 clip here is $6,284 of ADDITIONAL unhedged JPY on top of the existing Japan book.
- **Taxable**: tender proceeds are a disposition, not a rollover — short-term gain unless held >1y. On a +1.63% gross spread the after-tax return at a ~50% marginal rate is roughly half the headline.
- Source: https://www.release.tdnet.info/inbs/140120260730503195.pdf

### 2371 カカクコム — OPEN

- **Bidder / purpose**: Kamgras1株式会社 · GO_PRIVATE · target opinion SUPPORT_AND_RECOMMEND
- **Terms**: offer ¥3,450 vs spot ¥3,720 = -7.26% gross
- **Timing**: TIMING_UNBOUNDED — offer is PRE-CONDITIONAL (competition clearance etc.); the commencement date is NOT fixed, so a computed annualized number would be manufactured. Gross only. SCENARIO ONLY: at 180d to cash (from a house default 180d, since the bidder guided no window) the gross -7.26% annualizes to -15%/yr — an assumption, NOT a term of the offer, and it goes to zero if the conditions fail.
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥1436M/day
- **JPY add**: a ¥1,000,000 clip here is $6,284 of ADDITIONAL unhedged JPY on top of the existing Japan book.
- **Taxable**: tender proceeds are a disposition, not a rollover — short-term gain unless held >1y. On a -7.26% gross spread the after-tax return at a ~50% marginal rate is roughly half the headline.
...
- **8283 ＰＡＬＴＡＣ** — headline +0.30% vs offer ¥6,650. not quotable: last print 5d old — spread struck on a stale tape
...
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 1,418,000 issued shares, unit 百株
...
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 15,637,144 issued shares, unit 千株
...
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 22,500,000 issued shares, unit 千株
...
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 15,630,000 issued shares, unit 千株
...
- Minority float ~49% of shares; the controller needs to reach 90% to squeeze out without an EGM.
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 9,000,000 issued shares, unit 千株
...
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 38,315,000 issued shares, unit 千株
...
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 15,673,885 issued shares, unit 千株
...
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 29,990,400 issued shares, unit 株
...
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 135,452,804 issued shares, unit 千株
...
- Minority float ~49% of shares; the controller needs to reach 90% to squeeze out without an EGM.
- Inputs: annual-report balance sheet, price cached 6d ago — screen-grade, not valuation-grade. 大株主 table reconciled against 273,768,922 issued shares, unit 千株
