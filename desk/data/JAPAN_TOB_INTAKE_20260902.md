# Japan TOB / take-private lane — intake 2026-09-02

USD/JPY 159.82 at run. Announced book (spreads) and anticipatory book (fingerprints) are separate and are never summed.
...
54 deals tracked in the window; 16 live; 4 with a computable spread.

| target | name | bidder | offer ¥ | spot ¥ | gross % | ann. % | days | liquidity | timing |
|---|---|---|---|---|---|---|---|---|---|
| 4722 | フューチャー | 合同会社キーウェスト・ネットワー | 2,451 | 2,447 | +0.16 | +4 | 15 | QUOTABLE | DATED |
| 3276 | ＪＰＭＣ | 株式会社Amsterdam1及び | 2,250 | 2,247 | +0.13 | +2 | 23 | QUOTABLE | DATED |
| 9110 | ユナイテド海 | 郵船 | 10,600 | 10,470 | +1.24 | — | — | QUOTABLE | TIMING_UNBOUNDED |
| 2371 | カカクコム | ＬＩＮＥヤフー | 3,450 | 3,690 | -6.50 | — | — | QUOTABLE | TIMING_UNBOUNDED |

### 4722 フューチャー — OPEN

- **Bidder / purpose**: 合同会社キーウェスト・ネットワーク · GO_PRIVATE · target opinion not parsed
- **Terms**: offer ¥2,451 vs spot ¥2,447 = +0.16% gross
- **Timing**: DATED — cash expected 2026-09-17 (15d)
...
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥2224M/day
- **JPY add**: a ¥1,000,000 clip here is $6,257 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Terms**: offer ¥2,250 vs spot ¥2,247 = +0.13% gross
- **Timing**: DATED — cash expected 2026-09-25 (23d)
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥582M/day
- **JPY add**: a ¥1,000,000 clip here is $6,257 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Terms**: offer ¥10,600 vs spot ¥10,470 = +1.24% gross
- **Timing**: TIMING_UNBOUNDED — offer is PRE-CONDITIONAL (competition clearance etc.); the commencement date is NOT fixed, so a computed annualized number would be manufactured. Gross only. SCENARIO ONLY: at 119d to cash (from the bidder's own guided commencement window (2026-11-25)) the gross +1.24% annualizes to +4%/yr — an assumption, NOT a term of the offer, and it goes to zero if the conditions fail.
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥1432M/day
- **JPY add**: a ¥1,000,000 clip here is $6,257 of ADDITIONAL unhedged JPY on top of the existing Japan book.
- **Taxable**: tender proceeds are a disposition, not a rollover — short-term gain unless held >1y. On a +1.24% gross spread the after-tax return at a ~50% marginal rate is roughly half the headline.
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥910M/day
- **JPY add**: a ¥1,000,000 clip here is $6,257 of ADDITIONAL unhedged JPY on top of the existing Japan book.
- **Taxable**: tender proceeds are a disposition, not a rollover — short-term gain unless held >1y. On a -6.50% gross spread the after-tax return at a ~50% marginal rate is roughly half the headline.
...
- **2162 ｎｍｓ　ＨＤ** — headline +0.37% vs offer ¥540. not quotable: last print 6d old — spread struck on a stale tape
- **8283 ＰＡＬＴＡＣ** — headline +0.30% vs offer ¥6,650. not quotable: last print 26d old — spread struck on a stale tape
- **4171 グローバルＩ** — headline +0.90% vs offer ¥1,680. not quotable: last print 21d old — spread struck on a stale tape
...
- 7743 シード — TIMING_UNPARSED; QUOTABLE_SMALL; terms NOT parsed. ラックスシェア・プレシジョン・ケイマン・リミテッドによる当社株式に対する公開買付けに関する賛同の意見表明及び応募推奨に関するお知らせ
...
- 4420 イーソル — TIMING_UNPARSED; QUOTABLE_SMALL; terms NOT parsed. 株式会社BCJ-110によるイーソル株式会社（証券コード：4420）の普通株式に対する公開買付けの開始に関するお知らせ
