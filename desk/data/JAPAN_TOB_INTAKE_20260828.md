# Japan TOB / take-private lane — intake 2026-08-28

USD/JPY 159.64 at run. Announced book (spreads) and anticipatory book (fingerprints) are separate and are never summed.
...
52 deals tracked in the window; 14 live; 5 with a computable spread.

| target | name | bidder | offer ¥ | spot ¥ | gross % | ann. % | days | liquidity | timing |
|---|---|---|---|---|---|---|---|---|---|
| 2162 | ｎｍｓ　ＨＤ | ワールドＨＤ | 540 | 538 | +0.37 | +34 | 4 | THIN | DATED_STEP2 |
| 3276 | ＪＰＭＣ | 株式会社Amsterdam1及び | 2,250 | 2,249 | +0.04 | +1 | 28 | QUOTABLE | DATED |
| 4722 | フューチャー | 合同会社キーウェスト・ネットワー | 2,451 | 2,451 | +0.00 | +0 | 20 | QUOTABLE | DATED |
...
- **Timing**: DATED_STEP2 — step-2 cash-out effective 2026-09-01 (4d); delisting 2026-08-28
...
- **Liquidity (thin-print gate)**: THIN — 20d median turnover ¥8.2M/day — a quoted spread here is not fillable at size; this is a delisting stub, not a position
- **JPY add**: a ¥1,000,000 clip here is $6,264 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Terms**: offer ¥2,250 vs spot ¥2,249 = +0.04% gross
- **Timing**: DATED — cash expected 2026-09-25 (28d)
...
- **JPY add**: a ¥1,000,000 clip here is $6,264 of ADDITIONAL unhedged JPY on top of the existing Japan book.
- **Taxable**: tender proceeds are a disposition, not a rollover — short-term gain unless held >1y. On a +0.04% gross spread the after-tax return at a ~50% marginal rate is roughly half the headline.
...
- **Terms**: offer ¥2,451 vs spot ¥2,451 = +0.00% gross
- **Timing**: DATED — cash expected 2026-09-17 (20d)
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥2891M/day
- **JPY add**: a ¥1,000,000 clip here is $6,264 of ADDITIONAL unhedged JPY on top of the existing Japan book.
- **Taxable**: tender proceeds are a disposition, not a rollover — short-term gain unless held >1y. On a +0.00% gross spread the after-tax return at a ~50% marginal rate is roughly half the headline.
...
- **Timing**: TIMING_UNBOUNDED — offer is PRE-CONDITIONAL (competition clearance etc.); the commencement date is NOT fixed, so a computed annualized number would be manufactured. Gross only. SCENARIO ONLY: at 124d to cash (from the bidder's own guided commencement window (2026-11-25)) the gross +1.15% annualizes to +3%/yr — an assumption, NOT a term of the offer, and it goes to zero if the conditions fail.
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥1382M/day
- **JPY add**: a ¥1,000,000 clip here is $6,264 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **Squeeze-out**: step 2 = 株式併合 reverse split; requires an EGM special resolution, so a NON-tendering holder waits ~3-5 months beyond tender settlement for the same cash.
- **Liquidity (thin-print gate)**: QUOTABLE — 20d median turnover ¥947M/day
- **JPY add**: a ¥1,000,000 clip here is $6,264 of ADDITIONAL unhedged JPY on top of the existing Japan book.
...
- **8283 ＰＡＬＴＡＣ** — headline +0.30% vs offer ¥6,650. not quotable: last print 21d old — spread struck on a stale tape
- **4171 グローバルＩ** — headline +0.90% vs offer ¥1,680. not quotable: last print 16d old — spread struck on a stale tape
...
- 7743 シード — TIMING_UNPARSED; LIMIT_LOCKED; terms NOT parsed. ラックスシェア・プレシジョン・ケイマン・リミテッドによる当社株式に対する公開買付けに関する賛同の意見表明及び応募推奨に関するお知らせ
