...
def page(title, content, auth_script=False, signed_in=False, local=False):
    ...
    return ('<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">'
            ...
            '<img src="/public/mark.svg" width="29" height="29" alt="">worker placement.</a><nav aria-label="Main navigation">'
               '<a href="/guides/local">Documentation</a>' + ('<a href="/app/import">Bring your office</a>' if signed_in else '') + nav + '</nav></header>' + content +
...
def render_signup(finish=False, available=True, google_finish=False):
    ...
    content = ('<main id="main" class="auth-layout"><div class="auth-copy"><p class="eyebrow">WORKER PLACEMENT · HOSTED EARLY ACCESS</p><h1>' + title + '</h1><p>' + subtitle + '</p><p>Drop your statements, add your holdings and income, and set your goals using the same onboarding as the local app. An existing saved office can also be uploaded.</p></div>'
...
def render_local_guide(local=False):
    ...
    body = ('<main id="main" class="subpage"><p class="eyebrow">THE LOCAL SETUP GUIDE</p><h1>One script.<br>Your whole office.</h1>'
            ...
            '<h2 id="desktop-connections">Use IBKR TWS / IB Gateway from the website</h2>'
            '<p>These connections run on your computer. The local open-source app reads your broker, and office sync sends its saved records to your hosted office.</p>'
            '<ol class="guide-list"><li>Install Worker Placement using the command above on the computer running TWS or IB Gateway. Keep the broker API in read-only mode.</li>'
            '<li>If you already built an office online, export it from its <strong>Office settings</strong>, unzip into a separate folder, and start that same office with <code>./start.sh --dir /path/to/exported-office</code>. Keep its saved identity; do not create a second office with the same name.</li>'
            '<li>Open <strong>Hosting &amp; sync</strong> in the local app and sign in to the same Google account. For an existing hosted office, enable automatic sync and review any differences before choosing which copy to keep. Then import from the detected broker connection.</li>'
            '<li>If this is your first office, import your positions locally, review your goals and choose <strong>Build my office</strong>. In <strong>Hosting &amp; sync</strong>, review and upload it, open the hosted link, then enable automatic sync.</li></ol>'
            '<p>Leave the local app and broker connection running for continued updates. When they are offline, the website keeps the last imported data and its source dates. Broker login credentials stay local.</p>'
            '<p>For a hosted-only IBKR connection today, use Flex reports in Office settings, or upload a statement. Direct browser authorization is planned; it is not available yet.</p>'
            '<h2>Bring your office from the website</h2><p>Sign in and choose <strong>Upload a saved office</strong> from your workspace. Select the saved folder containing <code>answers.json</code> and <code>balance_sheet.json</code>, review the files, then choose <strong>Upload and open office</strong>. Your local copy stays available. An existing hosted office requires a reviewed replacement.</p>'
            '<h2>Create an office on the website</h2><p>Sign in, choose <strong>Create my office</strong>, then follow the same onboarding as the local app: drop statements or enter holdings, add income, and set life goals. Your progress is saved online. Connect an AI key in Office settings for conversational intake, then review the fields and choose <strong>Build my office</strong>.</p><h2>What is hosted today?</h2><p>The same onboarding and office workspace with private saved edits, retained documents and complete export, plus a shared SignalOS research library for every account. Connect your AI key and supported brokers in Office settings. Upload statements from Imports. Enable automatic sync in the local app to keep an offline copy; conflicting edits pause for review.</p>'
